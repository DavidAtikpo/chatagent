...
export default function LeadsPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-slate-900">{t("dashboard.leads.title")}</h1>
      <p className="mt-0.5 text-sm text-slate-600">
        {t("dashboard.leads.subtitleExtended", {
          count: qualifiedCount,
          plural: qualifiedCount > 1 ? "s" : "",
        })}
      </p>

      <div className="mt-2 flex flex-wrap gap-1.5">
        {[
          { value: 0, label: t("dashboard.leads.filterAll") },
          { value: 40, label: "≥ 40" },
          { value: 60, label: "≥ 60" },
          { value: 80, label: "≥ 80" },
        ].map((f) => (
      ...
      <div className="mt-3 overflow-hidden rounded-lg border border-slate-200 bg-white">
        {orgLoading || loading ? (
          ...
          <p className="p-4 text-sm text-slate-500">{t("dashboard.leads.emptyHint")}</p>
        ) : (
          <ResponsiveTable>
            <table className="w-full text-sm">
              <thead className="border-b border-slate-100 bg-slate-50 text-left text-xs text-slate-500">
                <tr>
                  ...
                  <th className="px-3 py-2">{t("dashboard.leads.contact")}</th>
                  <th className="px-3 py-2">{t("dashboard.leads.country")}</th>
                  <th className="px-3 py-2">{t("dashboard.leads.site")}</th>
                  <th className="px-3 py-2">{t("dashboard.leads.date")}</th>
                ...
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-slate-50 text-slate-800 hover:bg-slate-50">
                    <td className="px-3 py-2">
                      <span
                        className={`font-bold ${row.score >= 60 ? "text-brand-600" : "text-slate-600"}`}
                      >
                        {row.score}/100
                      </span>
                    </td>
                    <td className="px-3 py-2">
                      <div>{row.name ?? t("common.anonymousVisitor")}</div>
