...
export function DashboardTabs<T extends string>({
  ...
  return (
    <div
      className={`flex gap-0.5 overflow-x-auto rounded-md bg-slate-100 p-0.5 ${className}`}
      role="tablist"
    >
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={
              selected
                ? "shrink-0 rounded px-2.5 py-1 text-xs font-semibold text-slate-900 shadow-sm bg-white"
                : "shrink-0 rounded px-2.5 py-1 text-xs font-medium text-slate-600 hover:text-slate-900"
            }
          >
            {tab.label}
          </button>
...
export function DashboardPanel({
  ...
  return (
    <section className={`rounded-lg border border-slate-200 bg-white ${className}`}>
      <div className="flex flex-wrap items-start justify-between gap-1.5 border-b border-slate-100 px-3 py-2">
        <div>
    ...
    <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-4">
      {items.map((item) => (
        <div
          ...
          className={
            item.highlight
              ? "rounded-md bg-brand-50 p-2 ring-1 ring-brand-100"
